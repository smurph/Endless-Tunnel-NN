namespace Assets.Scripts
{
    ...
    class DynamicWall : MonoBehaviour
    {
        ...
        public void UpdateMesh(float gap)
        {
            var offsets = Perlin.Instance.Offsets;
            var filter = GetComponent<MeshFilter>();
            var vertices = filter.mesh.vertices;

            for (int i = 0; i < offsets.Count; i++)
            {
                float y = (gap / 2) + offsets[i];

                if (!IsUpperWall) y -= gap;
            ...
            //GetComponent<PolygonCollider2D>(). = filter.sharedMesh;
        }
